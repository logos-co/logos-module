#!/usr/bin/env bash
#
# Execute every lm doc-test end-to-end and regenerate its Markdown.
#
...
# a temp directory (building lm, compiling a plugin, assembling module
# directories, packaging and verifying them) and asserts on the output;
# `doctest generate` renders the same spec to Markdown under outputs/ (the
# *.test.yaml spec is the source of truth).
#
...
# published flake. Each spec's `github:logos-co/logos-module{release}` URL is
# pinned to $COMMIT via --release-for, so the run exercises exactly what's
...
# to logos-co/logos-module. A local-only / uncommitted HEAD won't resolve;
# export COMMIT="" (or push first) in that case.
COMMIT="${COMMIT-$(git rev-parse HEAD)}"
RELEASE_FOR=()
if [ -n "${COMMIT}" ]; then
  RELEASE_FOR=(--release-for "logos-module=${COMMIT}")
  echo "==> Pinning logos-module to ${COMMIT}"
else
  echo "==> COMMIT empty; building logos-module from latest"
fi
